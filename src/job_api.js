...
class JobAPI {
    ...
    authorizeMiner(minerId, username, password) {
        ...
        // Parse Password for Solana Address ("x,solanaAddress=...")
        if (password) {
            const parts = password.split(',');
            for (const part of parts) {
                if (part.trim().startsWith('solanaAddress=')) {
                    miner.solanaAddress = part.trim().split('=')[1];
                    console.log(`Miner ${miner.id} registered Solana Address: ${miner.solanaAddress}`);
                }
            }
        }
        return true;
